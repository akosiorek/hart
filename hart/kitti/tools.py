...
def resample_sequence_dict(data, max_len, overlap, min_length=0):
...
def sequences(data, max_len=None, shuffle=True, overlap=0, max_objects=None, sample_objects=False):
    ...
    if max_objects is not None:
        ...
        if sample_objects < 0:
            choice_fun = lambda x, n: sample_permutations(x, n, max(1, abs(sample_objects) * (x.shape[0] // max_objects)))
...
class KittiStore(store.CircularValueStore):
    ...
    def __init__(self, data_dict, n_timesteps, img_size, batch_size, overlap_fraction=.5,
                 max_objects=1, sample_objects=False, num_epochs=None, shuffle=True,
                 ...
                 deplete_queues_at_length_increase=False):
    ...
    def set_length(self, n_timesteps, sess=None):
        if self.n_timesteps != n_timesteps:
            ...
            data = sequences(data, n_timesteps, False, overlap,
                             max_objects=self.max_objects, sample_objects=self.sample_objects)
    ...
    def get_minibatch(self):
        ...
        if self.minibatch is None:
            ...
            def get_single_sample():
                return process_entry(self.get(), self.max_objects, self.img_store,
                                     self.depth_folder, self.bbox_scale)

            n_channels = 3 + int(self.depth_folder is not None)
            shapes = [(None,) + tuple(self.img_size) + (n_channels,), (None, self.max_objects, 4),
                      (None, self.max_objects)]
            dtypes = [self.storage_dtype, tf.float32, tf.uint8]
            names = ['img', 'bbox', 'presence']

            sample, sample_queue_size = data_tools.run_py2tf_queue(get_single_sample, dtypes, shapes=shapes,
                                                        names=names, n_threads=self.n_threads, capacity=2 * self.batch_size,
...
def get_data(img_folder, label_folder, train_fraction, img_size,
             train_timesteps=4, test_timesteps=4, batch_size=1, max_objects=1, sample_objects=False, n_threads=3,
             in_memory=False, which_seqs=None, truncated_threshold=2., occluded_threshold=3., depth_folder=None,
             storage_dtype=tf.uint8, mirror=False, reverse=False, bbox_scale=.5, partition='all'):

    partition = partition.lower()
    assert partition in ('train', 'test', 'all'), 'Unknown partition: {}.'.format(partition)
    ...
    def make_store(d, timesteps, n_threads, mirror=False, reverse=False, name=''):
        s = KittiStore(d, timesteps, img_size, batch_size, max_objects=max_objects,
    ...
    if partition == 'train' or partition == 'all':
        train_store = make_store(train, train_timesteps, n_threads, mirror, reverse, name='train')
        result = [train_store, train_store.get_minibatch()]
    elif partition == 'test':
        test_store = make_store(test, test_timesteps, n_threads, name='test')
        result = [test_store, test_store.get_minibatch()]

    if partition == 'all':
        test_store = make_store(test, test_timesteps, (n_threads // 2) + 1, name='test')
        result.extend((test_store, test_store.get_minibatch()))
...
if __name__ == '__main__':
    ...
    img_size = (187, 621)
    img_folder = '../../../../data/kitti_tracking/small'
    label_folder = '../../../../data/kitti_tracking/label_02'
    keys = ['img', 'bbox', 'presence']

    train_store, train, test_store, test = get_data(img_folder, label_folder, .8, img_size, 2, 2,
                           batch_size=2, max_objects=3, sample_objects=-4, n_threads=2, in_memory=False)

    train_minibatch = {k: train[k] for k in keys}
    test_minibatch = {k: test[k] for k in keys}
    print 'Train samples =', train_store.n_batches_per_epoch
    print 'Test samples =', test_store.n_batches_per_epoch
    ...
    for i in xrange(n_itr):
        start0 = time.time()
        out_dict = sess.run(train_minibatch)
        imgs, bboxes, presence = (out_dict[k] for k in keys)
        print i, imgs.shape, bboxes.shape, presence.shape

        print time.time() - start
    print 'avg', (time.time() - start) / n_itr
